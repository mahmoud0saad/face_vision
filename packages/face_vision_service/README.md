...
Copy the entire `packages/face_vision_service/` folder (including `lib/assets/models/`), add the same `path:` (or publish as a git dependency), and run `dart pub get`.
...
1. Copy `packages/face_vision_service/` into your repo (models included under `lib/assets/models/`).
2. Add path dependency in `pubspec.yaml`.
3. For Flutter, wire `readBytes` as in [Model loading in Flutter](#model-loading-in-flutter).
4. Use `FaceVisionServiceClient` as in [Quick start](#quick-start).

No changes inside `lib/src/` are required unless you want different models or thresholds.

## License

Same as the parent `face_vision` project.
